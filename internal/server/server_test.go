...
func TestCompareVersions(t *testing.T) {
    if compareVersions("v1.2.10", "1.2.3") <= 0 { t.Fatalf("expected 1.2.10 > 1.2.3") }
    if compareVersions("141000", "140900") <= 0 { t.Fatalf("expected 141000 > 140900") }
    if compareVersions("1.4.1.0", "1.4.0.9") <= 0 { t.Fatalf("expected 1.4.1.0 > 1.4.0.9") }
    if compareVersions("1.0.0", "1.0.0") != 0 { t.Fatalf("expected equal") }
}
...
func TestPickLatestStable(t *testing.T) {
    ...
    if s.latest["fcl"] != "v1.2.3" { t.Fatalf("expected latest stable v1.2.3, got %s", s.latest["fcl"]) }

    rr := httptest.NewRecorder()
    req := httptest.NewRequest(http.MethodGet, "/api/status", nil)
    s.handleStatus(rr, req)
    if got := rr.Header().Get("X-Latest-Versions"); got == "" || got != "fcl=v1.2.3" { t.Fatalf("unexpected header: %q", got) }
}
...
func TestLatestEndpointAndRollback(t *testing.T) {
    ...
    if got := rr.Header().Get("X-Latest-Version"); got != "141000" { t.Fatalf("unexpected header: %q", got) }

    s.RemoveVersion("zl", "141000")
    rr2 := httptest.NewRecorder()
    req2 := httptest.NewRequest(http.MethodGet, "/api/latest/zl", nil)
    s.handleLatestLauncher(rr2, req2)
    if got := rr2.Header().Get("X-Latest-Version"); got != "140900" { t.Fatalf("rollback header: %q", got) }
}
...
func TestLatestAllEndpoint(t *testing.T) {
    ...
    if rr.Code != http.StatusOK { t.Fatalf("status %d", rr.Code) }
    if got := rr.Header().Get("X-Latest-Versions"); got == "" { t.Fatalf("missing header") }
}